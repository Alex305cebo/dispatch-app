...
- brokerEmail = WHERE THE INVOICE AND PAPERWORK GO, when the document names such an address — the billing / remittance / "documents required for payment" address (e.g. "ICS_scan@jbhunt.com", "QPdocs@jbhunt.com", billing@, invoices@, ap@, docs@). That address is what the dispatcher needs to get paid; a rep's mailbox is not where an invoice is processed. Only if the document names NO such address, fall back to the rep's contact email. Put the rep's own email and direct line in importantNotes under [CONTACT] either way.
- brokerPhone = the best phone printed for that brokerage; prefer a carrier-support / payment line when one is printed, otherwise the rep's direct line.
- payVia = the NAME of the service or portal through which this broker takes invoices and pays carriers, if the document names one — a third-party payment service (TriumphPay, Comdata, RTS, Apex, Triumph, OTR, eCapital, Denim) OR the broker's own carrier portal when paperwork is submitted there (e.g. "Carrier 360" for J.B. Hunt, "Carrier Hub", "Navisphere"). Rate cons state this in the payment/remittance block, e.g. "All carrier payments are now being processed through triumphpay.com" -> "TriumphPay". Other common ones: Comdata, RTS, Apex, Triumph, OTR, eCapital, Denim. Also use it for an assignment notice ("this invoice has been assigned to ..."). Just the company name, no sentence. null if the document says nothing about how payment is processed — most do not.
