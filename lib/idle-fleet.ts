 ...
 *
 * Порядок: дольше всех стоящие сверху — им искать груз в первую очередь. Дальше
 * те, кто скоро освободится (чем скорее, тем выше: под них груз ищут заранее).
 * Ремонт и отпуск уходят в конец: они стоят денег, но их нельзя загрузить.
 */
export function idleFleet(
  ...
  return rows.sort((a, b) => {
    ...
    // Среди свободных: дольше стоит — выше. Без истории (null) считаем самым давним,
    // иначе новый трак, ни разу не гружённый, прятался бы внизу списка.
    if (a.free) return (b.days ?? Infinity) - (a.days ?? Infinity)
