...
const CAPTION_KINDS: { kind: DocClass; re: RegExp }[] = [
  ...
  // Пломба — пластиковый замок с номером на дверях прицепа, отдельный снимок.
  // Слово короткое и многозначное («sealed bid»), поэтому только целым словом.
  // Оговорка в начале — про накладную: номер пломбы печатается в самом BOL, и
  // подпись «BOL seal 4471» относится к накладной. Такую подпись отдаём глазам
  // модели, как и раньше, вместо того чтобы назвать снимком замка.
  {
    kind: 'seal',
    re: /^(?!.*\bbol(?![a-z]))(?!.*\bbill[\s._-]*of[\s._-]*lading).*(?:\bseals?(?![a-z])|пломб)/is,
  },
]
