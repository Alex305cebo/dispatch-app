...
export default async function BrokerPage({ params }: { params: Promise<{ key: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 pb-20 pt-6 sm:px-6 sm:pt-10">
      ...
      {b.loadCount === 0 ? (
        <p className="mt-4 text-base text-t3">{t(locale, 'brokers.card.none')}</p>
      ) : (
        <>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            ...
            <section className="panel min-w-0 p-4">
              ...
              <ul className="nums flex flex-col gap-1 text-base text-t1">
                {lanes.map(([lane, n]) => (
              ...
              {facilities.length > 0 && (
                <>
                  <h3 className="mb-1 mt-3 text-sm font-medium text-t3">{t(locale, 'brokers.card.facilities')}</h3>
                    ...
                    <div key={l.id} className="flex flex-wrap items-center gap-x-3 gap-y-0.5 rounded-lg border border-white/8 px-3 py-2 text-base">
                      ...
                      {u ? (
                        <Link
                          ...
                          className={`nums text-sm hover:underline ${u.days > 30 ? 'text-warn-400' : 'text-t3'}`}
                        >
