    ...
    <div className="mt-3 rounded-xl border border-white/8 bg-ink-950/50 p-3">
      ...
      <div className="mt-2 flex flex-wrap items-end gap-2">
        <label className="block">
          <span className="mb-0.5 block text-xs text-t2 font-medium">
        ...
        <div className="ml-auto text-right">
          <div className="nums text-[19px] font-bold text-t1">{usd.format(cost)}</div>
          <div className="text-xs text-t2 font-medium">{t(locale, 'tolls.nyc.total')}</div>
        </div>
      </div>
    </div>
  )
