...
// Streams a stored document. Behind the PIN gate (middleware) — the browser sends
// the auth cookie on normal navigation, so links "just work" for a signed-in user.
export async function GET(
  ...
  const rows = await sql`
    SELECT title, mime, encode(data, 'base64') AS b64 FROM documents WHERE id = ${Number(id)}`
