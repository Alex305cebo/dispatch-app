 ...
 * поэтому пишем в settings, а не в браузер, и менять её может только администратор
 * (решение владельца 23.09.2026): диспетчер не сдвигает экран всей компании.
 *
...
/**
 * Реквизиты крупного брокера из справочника «Крупнейшие брокеры» — по реестру SAFER,
 * а не из списка: вписанный руками MC устаревает и врёт. Вернулось на «Рынок»
 * 19.09.2026 вместе с самим справочником (убирали 16.09).
 */
export async function topBrokerInfo(name: string): Promise<TopFacts | { error: string }> {
  ...
  if (!best && cards.length > 0) {
    // В реестре несколько компаний с этим именем. Для автозаписи MC в грузы такая
    // ничья отдаётся человеку, но здесь справочная карточка ИЗВЕСТНОГО брокера —
    // выбираем сами, сужая шаг за шагом: штат штаб-квартиры из нашего справочника →
    // брокерский авторитет → дословное имя → старейший DOT (однофамильцы-подражатели
    // регистрируются недавно, номера у них большие).
    let pool = cards
    if (known?.hq) {
...
export async function quoteBoardLane(label: string, miles: number, loadId?: number): Promise<{ rpm: number } | { error: string }> {
  ...
  const res = await saveWarpLane(companyId, fromCity, toCity, miles, locale)
  if ('error' in res) return res
  revalidatePath('/trucks')
  if (loadId != null) revalidatePath(`/loads/${loadId}`)
  return res
}
