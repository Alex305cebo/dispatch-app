    ...
    <div className="mb-4 rounded-xl border border-white/10 bg-white/[0.03] p-3">
        ...
        <ul className="mt-2 divide-y divide-white/[0.06]">
          {charges.map((c) => (
            <li key={c.id} className="flex items-center gap-2 py-1.5 text-[13px]">
              <span className="font-medium text-white/85">{t(locale, KIND_KEY[c.kind])}</span>
              {c.note && <span className="min-w-0 flex-1 truncate text-white/55">{c.note}</span>}
              ...
              <button
                ...
                className="flex size-7 shrink-0 items-center justify-center rounded-md text-white/40 transition-colors hover:bg-bad-500/15 hover:text-bad-400 disabled:opacity-50 max-md:size-9"
              >
      ...
      {charges.length > 0 && (
        <p className="mt-2 flex items-baseline justify-between border-t border-white/10 pt-2 text-[13px]">
          <span className="text-white/65">{t(locale, 'loads.charges.total')}</span>
          <span className="nums font-bold">
            {usd.format(rate + total)} <span className="text-[11px] font-medium text-white/45">= {usd.format(rate)} + {usd.format(total)}</span>
          </span>
        </p>
      )}
