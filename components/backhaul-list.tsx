          ...
          <li key={b.key} className="py-1.5 first:pt-0 last:pb-0">
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
              <div className="flex min-w-0 flex-1 basis-[14rem] flex-wrap items-baseline gap-x-2">
                <Link href={`/brokers?q=${encodeURIComponent(b.mc ?? b.name)}`} className="text-base font-medium hover:underline">
                  {b.name}
                </Link>
                {b.mc && <span className="nums text-xs text-t3">MC {b.mc}</span>}
              ...
              <div className="flex shrink-0 gap-1">
                {b.phone && (
                  <a href={`tel:${b.phone}`} className="rounded-md bg-haul-500/15 px-2 py-0.5 text-sm font-semibold text-haul-300 hover:bg-haul-500/25 max-md:py-1.5">
                    📞 {b.phone}
                  </a>
                )}
                {b.email && (
                  <a href={`mailto:${b.email}`} className="rounded-md border border-white/12 px-2 py-0.5 text-sm font-medium text-t2 hover:border-white/30 max-md:py-1.5">
                    ✉
                  </a>
                )}
              </div>
