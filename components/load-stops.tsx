  ...
  return (
    <div className="panel-inset min-w-0 px-3 py-2.5">
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
        ...
        {when && <span className="nums min-w-0 text-base font-semibold text-t1">{when}</span>}
        {nav && (
          <a
            href={nav}
            target="_blank"
      ...
      {(stop.name || stop.address || city) && (
        <p className="mt-1 text-sm leading-snug break-words text-t2">
          {stop.name && <span className="text-base font-semibold text-t1">{stop.name}</span>}
        ...
        <div className="mt-1.5 flex flex-wrap gap-1">
          {stop.refs.map((r) => (
            <button
              ...
              className="nums inline-flex items-center gap-1 rounded-md border border-white/10 bg-white/[0.05] px-1.5 py-0.5 text-xs font-semibold text-t1 hover:border-haul-500/40"
            >
              {r}
