...
export function Empty({
  ...
  if (row) {
    return (
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        <span className="flex size-8 shrink-0 items-center justify-center rounded-xl bg-white/[0.05] text-t3 ring-1 ring-white/8">
  ...
  return (
    ...
    >
      ...
      <span className="mb-1 flex size-11 items-center justify-center rounded-2xl bg-white/[0.05] text-t3 ring-1 ring-white/8">
        <Icon size={20} strokeWidth={1.75} />
      </span>
      <p className="text-lg font-semibold text-t1">{title}</p>
      {text && <p className="max-w-sm text-base leading-relaxed text-t3">{text}</p>}
      {action && (
