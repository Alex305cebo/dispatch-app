      ...
      <section className="panel p-4">
        ...
        {editMeta && (
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            ...
            <label className="block">
              <span className="mb-1.5 block text-xs font-medium text-t2">{t(locale, 'trucks.care.homeStateLabel')}</span>
              <select
                value={m.homeState}
          ...
          <ul className="mt-3 flex flex-col gap-1.5">
            {todos.map((t) => (
              <li key={t.id} className="flex items-center gap-3 rounded-lg border border-white/6 px-3 py-2">
                ...
                <DeleteButton
                  action={(id, confirm) => deleteTodo(id, truckId, confirm)}
                  id={t.id}
                  title={t.title}
                />
              </li>
                ...
                <li key={r.id} className="rounded-lg border border-white/6 px-3 py-2">
                  <div className="flex items-baseline justify-between gap-3">
                    ...
                    <span className="flex shrink-0 items-center gap-2">
                      ...
                      <DeleteButton
                        action={(id, confirm) => deleteMaintenance(id, truckId, confirm)}
                        id={r.id}
                        title={r.title}
                      />
                    </span>
