...
export async function DemoModeBanner() {
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-40 flex h-9 items-center justify-center gap-2 overflow-hidden whitespace-nowrap bg-haul-500 px-3 text-center text-[11.5px] font-medium text-white md:pl-52 md:text-[12px]">
      <span className="hidden md:inline">🧪 {t(locale, 'demo.banner')}</span>
      <span className="md:hidden">{t(locale, 'demo.bannerShort')}</span>
