...
//
// Забирает ставки по регионам и грузы на трак по штатам для Van, Reefer и Flatbed плюс
// цену дизеля, проверяет форму ответов теми же разборщиками, что и приложение
...
import { parseFuel, parseLt, parseRegions } from '../lib/dat-market-core.ts'
...
for (const equipment of SERIES) {
  ...
  console.error(`${equipment}: регионов ${regions.length}, штатов ${Object.keys(lt).length}`)
  sql.push(put(`dat_trendlines_${equipment}`, JSON.stringify({ equipment, at, regions, lt, fuel })))
}
if (!sql.length) {
